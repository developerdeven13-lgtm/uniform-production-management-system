          ...
          <Link
            ...
            style={{
              ...
              ...(!statusFilter && !priorityFilter
                ? { background: '#0f2416', color: '#fff', borderColor: 'transparent' }
                : { background: 'transparent', color: '#888780', borderColor: '#D3D1C7' }),
            }}
          >
            All
          </Link>
          ...
          <Link
            ...
            style={{
              ...
              ...(priorityFilter === 'urgent'
                ...
                : { background: 'transparent', color: '#791F1F', borderColor: '#F7C1C1' }),
            }}
          >
            ⚠ Urgent
          </Link>

          <div style={{ width: '0.5px', height: 18, background: '#D3D1C7', margin: '0 2px' }} />
      ...
      >
        {!orders || orders.length === 0 ? (
          <div style={{ padding: '48px 24px', textAlign: 'center' }}>
            <div style={{ width: 40, height: 40, background: '#F1EFE8', borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 12px' }}>
              <Plus className="w-5 h-5" style={{ color: '#888780' }} />
            </div>
            <p style={{ fontSize: 13, fontWeight: 500, color: '#2C2C2A' }}>No orders found</p>
            <p style={{ fontSize: 11, color: '#888780', marginTop: 4, marginBottom: 16 }}>
              {statusFilter || priorityFilter ? 'Try adjusting your filters.' : 'Get started by creating your first order.'}
            </p>
            {!statusFilter && !priorityFilter && (
              <Link
                href="/orders/new"
                style={{ fontSize: 12, fontWeight: 500, color: '#0f2416', textDecoration: 'none' }}
              >
                + Create first order
              </Link>
            )}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr style={{ borderBottom: '0.5px solid #F1EFE8', background: '#F7F5EE' }}>
                {['Order #', 'Customer', 'Status', 'Items', 'Delivery', 'Created'].map((h, i) => (
                  <th
                    key={h}
                    className={i >= 3 ? (i === 3 ? 'hidden sm:table-cell' : 'hidden md:table-cell') : ''}
                    style={{
                      textAlign: 'left',
                      padding: '10px 18px',
                      fontSize: 9,
                ...
                <ClickableOrderRow key={order.id} orderId={order.id}>
                  <td style={{ padding: '12px 18px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      <span style={{ fontFamily: 'monospace', fontSize: 12, fontWeight: 700, color: '#2C2C2A', letterSpacing: '-0.3px' }}>
                        {order.order_number}
                      </span>
                      {order.priority === 1 && (
                        <span style={{ fontSize: 9, padding: '2px 7px', background: '#2C2C2A', color: '#F1EFE8', borderRadius: 99, fontWeight: 500 }}>
                          Urgent
                        </span>
                      )}
                    </div>
                  ...
                  <td style={{ padding: '12px 18px' }}>
                    <p style={{ fontSize: 13, fontWeight: 600, color: '#2C2C2A' }}>{order.customer?.full_name ?? '—'}</p>
                    <p style={{ fontSize: 11, color: '#888780' }}>{order.customer?.phone}</p>
                  </td>
                  <td style={{ padding: '12px 18px' }}>
                    <OrderStatusBadge status={order.status as OrderStatus} />
                  </td>
                  <td className="hidden sm:table-cell" style={{ padding: '12px 18px', fontSize: 12, color: '#5F5E5A' }}>
                    {order.total_items}
                  </td>
